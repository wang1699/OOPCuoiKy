...
public class LoginHocVienGUI extends JFrame {
	...
	public LoginHocVienGUI(String maTaiKhoan) {
		...
		String maHocVien = dao.getMaHocVIenByTaiKhoan(maTaiKhoan);
                ...
                if (e.getClickCount() == 2 && table.getSelectedRow() != -1) {
                    ...
                    if (luaChon == JOptionPane.YES_OPTION) {
                        DangKyDAO dangKyDAO = new DangKyDAO();
                         dangKyDAO.dangKyKhoaHoc(maHocVien, maKhoaHoc);
		...
		btn_TimKiem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (DiemSo d : danhSachDiem) {
					model.addRow(new Object[] { d.getMaKhoaHoc(), d.getDiem() });
				}
		...
		JButton btn_TimKiem_1 = new JButton("Đăng ký khóa học mới");
		btn_TimKiem_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				List<KhoaHoc> danhSachChuaDangKy = khoaHocDAO.layKhoaHocChuaDangKy(maHocVien);
				if (danhSachChuaDangKy.isEmpty()) {
					model.setRowCount(0);
					JOptionPane.showMessageDialog(null, "Học viên không còn khóa học nào để đăng ký!", "Thông báo",
					...
					model.setRowCount(0);
					model.setColumnIdentifiers(new String[] { "Mã Khóa Học", "Tên Khóa Học", "Số Tín Chỉ", "Học Phí" });
					for (KhoaHoc kh : danhSachChuaDangKy) {
						model.addRow(new Object[] { kh.getMaKhoaHoc(), kh.getTenKhoaHoc(), kh.getSoTinchi(),
								kh.getHocPhi() });
					}
				}
		...
		btn_TimKiem_1.setBounds(727, 129, 195, 40);
